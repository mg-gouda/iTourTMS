...
export function generateContractPdf(
  ...
  const HEADER_H = 44; // reserved height for page header
  const HEADER_BOTTOM = margin.top + HEADER_H;
  const SIGNATURE_H = 28; // reserved height for signature block
  const CONTENT_BOTTOM = pageH - margin.bottom;

  let cursorY = HEADER_BOTTOM;

  // Track pages that have headers drawn (drawn at end)
  // autoTable margin.top is set to HEADER_BOTTOM so new pages leave room
  ...
  // ─── Signature block ────────────────────────────────────
  function drawSignatureBlock(): void {
    const needed = SIGNATURE_H;
    if (cursorY + needed > CONTENT_BOTTOM) {
      doc.addPage();
      cursorY = HEADER_BOTTOM;
    }
    const sigY = cursorY + 8;
    const leftX = margin.left + 15;
    ...
    doc.text("Hotelier's Signature", leftX, sigY);
    doc.text("Buyer's Signature", rightX, sigY);

    doc.setDrawColor(0, 0, 0);
    doc.setLineWidth(0.3);
    doc.line(leftX, sigY + 10, leftX + lineW, sigY + 10);
    doc.line(rightX, sigY + 10, rightX + lineW, sigY + 10);

    doc.setFont("helvetica", "italic");
    doc.setFontSize(6.5);
    doc.setTextColor(TEXT_MUTED.r, TEXT_MUTED.g, TEXT_MUTED.b);
    doc.text(
      `Signed on behalf of ${data.hotel.name}`,
      leftX,
      sigY + 14,
    );
    doc.text(
      `Signed on behalf of ${options.companyName}`,
      rightX,
      sigY + 14,
    );
  ...
  function drawPageHeader(pageIndex: number): void {
    ...
    // Row 3: Full-width info table (2 columns of key-value pairs)
    const marketsText =
    ...
    const infoBody = [
      ...
      [
        "Contract Date",
        fmtDate(data.validFrom),
        "Status",
        CONTRACT_STATUS_LABELS[data.status] ?? data.status,
      ],
      [
        "Currency",
      ...
      [
        ...
        "Stay Dates",
        `${fmtDate(data.travelFrom)} TO ${fmtDate(data.travelTo)}`,
      ],
      [
        "Markets",
        marketsText,
  ...
  // ─── Force new section page ─────────────────────────────
  function newSectionPage(): void {
    doc.addPage();
    cursorY = HEADER_BOTTOM;
  }

  // ─── Add page footers + headers on all pages ───────────
  ...
  // =====================================================================
  // PAGE 1: Room Types, Rates & Supplements
  // =====================================================================
  ...
  // (Seasons moved to card grid after rates)

  // ── Rates Matrix ──
  if (data.baseRates.length > 0 && data.roomTypes.length > 0) {
    sectionTitle("Rates");
    ...
    // Helper: find a supplement value for a given room type + type + criteria
    function findSupp(
      suppType: string,
      roomTypeId: string | null,
      seasonId: string | null,
      ...
      // Try season + room specific, then season + global room, then global
      const matches = data.supplements.filter((s) => {
        if (s.supplementType !== suppType) return false;
        if (criteria.forAdults != null && s.forAdults !== criteria.forAdults)
          return false;
        if (
          criteria.childPosition != null &&
          s.childPosition !== criteria.childPosition
        )
          return false;
        return true;
      });

      // Prefer: season+room > season+noRoom > noSeason+room > noSeason+noRoom
      const rank = (s: (typeof matches)[number]) => {
        let r = 0;
        if (s.seasonId === seasonId) r += 2;
        else if (s.seasonId != null) return -1; // wrong season
        if (s.roomTypeId === roomTypeId) r += 1;
        else if (s.roomTypeId != null) return -1; // wrong room
        return r;
      };
      ...
      return {
        ...
        isReduction: best.isReduction,
      };
    }

    // Compute Base PP for a room type in a season
    function getBasePP(
      rt: (typeof data.roomTypes)[number],
      seasonId: string,
    ): number {
      const baseRate = rateMap.get(seasonId);
      if (!baseRate) return 0;
      const base = Number(baseRate.rate);

      if (rt.isBase) return base;

      // Find room type supplement
      const rtSupp = data.supplements.find((s) => {
        if (s.supplementType !== "ROOM_TYPE") return false;
        if (s.roomTypeId !== rt.roomTypeId) return false;
        if (s.seasonId != null && s.seasonId !== seasonId) return false;
        return true;
      });

      if (!rtSupp) return base;

      const suppVal = Number(rtSupp.value);
      if (rtSupp.valueType === "PERCENTAGE") {
        return rtSupp.isReduction
          ? base * (1 - suppVal / 100)
          : base * (1 + suppVal / 100);
      }
      return rtSupp.isReduction ? base - suppVal : base + suppVal;
    }

    const subCols = ["Base PP", "SGL Sup\n%", "03rd AD\nRate Red", "01st Chd\n%", "02nd Chd\n%"];
    const RT_PER_TABLE = 4;

    // Split room types into chunks of 4
    ...
    for (let chunkIdx = 0; chunkIdx < rtChunks.length; chunkIdx++) {
      ...
      // Header row 2: From, To + sub-column labels per room type
      const headRow2: any[] = [
      ...
      for (const season of data.seasons) {
        const row: string[] = [
          fmtDate(season.dateFrom),
          fmtDate(season.dateTo),
        ];
      ...
      // First extra chunk starts on a new page; subsequent ones flow naturally
      if (chunkIdx === 1) {
        doc.addPage();
        cursorY = HEADER_BOTTOM;
    ...
    doc.setTextColor(TEXT_MUTED.r, TEXT_MUTED.g, TEXT_MUTED.b);
    doc.text(
      `Base PP — Base Rate Per Person Per Night, SGL Sup % — Single Supplement, 03rd AD Rate Red — Third Adult Rate Reduction, 01st/02nd Chd % — Child Percentage. All rates in ${data.baseCurrency.code}.`,
      margin.left,
      cursorY,
    );
    cursorY += 3;
  ...
  // =====================================================================
  // CARD GRID: Compact info sections after Rates (2 columns)
  // =====================================================================

  // ─── Reusable: draw a card table at specific position ───
  function drawCard(
    title: string,
    head: string[],
    body: string[][],
  ...
  // ─── Reusable: draw a key-value card (like special offer cards) ───
  function drawKvCard(
    title: string,
    rows: string[][],
    cardX: number,
  ...
  // Build all cards as objects so we can lay them out in a 2-column grid
  interface CardDef {
    type: "table" | "kv";
    title: string;
    head?: string[];
    ...
    columnStyles?: Record<
      ...
      { halign?: "left" | "center" | "right"; cellWidth?: number }
    >;
    estimatedRows: number; // for height estimation
  }
  ...
  // 3. Rate / Room Calculation
  {
    ...
    for (const rt of data.roomTypes) {
      ...
      const supp = data.supplements.find(
        (s) =>
          ...
          s.roomType?.name === rt.roomType.name,
      );
      if (supp) {
    ...
    if (calcRows.length > 0) {
      cards.push({
        type: "kv",
        title: "Rate / Room Calculation",
        body: calcRows,
        estimatedRows: calcRows.length,
      });
    }
  }
  ...
  if (data.cancellationPolicies.length > 0) {
    function formatCharge(
      cp: ContractPdfData["cancellationPolicies"][number],
    ): string {
  ...
  // 8. Special Offers (as compact kv cards in the same grid)
  for (const so of data.specialOffers) {
    ...
    if (so.combinable !== undefined) {
      soRows.push(["Combinable", so.combinable ? "Yes" : "No"]);
    }
    if (so.validFrom || so.validTo) {
      soRows.push(["Stay Dates", `${fmtDate(so.validFrom)} – ${fmtDate(so.validTo)}`]);
    }
    if (so.bookFromDate || so.bookByDate) {
      soRows.push(["Booking Dates", `${fmtDate(so.bookFromDate)} – ${fmtDate(so.bookByDate)}`]);
    }
    if (so.stayDateType) {
      soRows.push(["Stay Date Type", so.stayDateType]);
    }
    if (so.minimumNights) {
      soRows.push(["Min Nights", String(so.minimumNights)]);
    }
    if (so.paymentPct && so.paymentDeadline) {
      soRows.push(["Payment", `${so.paymentPct}% by ${fmtDate(so.paymentDeadline)}`]);
    }
    if (so.roomingListBy) {
      soRows.push(["Rooming List By", fmtDate(so.roomingListBy)]);
    }
  ...
  if (cards.length > 0) {
    ...
    const ROW_HEIGHT_EST = 3.5; // estimated mm per row
    const CARD_OVERHEAD = 10; // header rows + padding

    // Place cards in 2-column layout
    let colIndex = 0;
    ...
    for (const card of cards) {
      ...
      // If we're at col 0 and won't fit, start new page
      ...
      // If we're at col 1 and won't fit, wrap to next row first
      if (colIndex === 1 && rowStartY + estH > CONTENT_BOTTOM) {
        // Move to next row on a new page
        doc.addPage();
        rowStartY = HEADER_BOTTOM;
        rowMaxBottom = HEADER_BOTTOM;
        cursorY = HEADER_BOTTOM;
      ...
      if (card.type === "kv") {
        ...
        finalY = drawCard(
          ...
          rowStartY,
          cardW,
          { columnStyles: card.columnStyles },
        );
      }
    ...
    // If we ended mid-row, update cursor
    if (colIndex > 0) {
      cursorY = rowMaxBottom + GAP;
    }
  }
  ...
  if (data.terms || data.internalNotes) {
    newSectionPage();
    ...
    if (data.terms) {
      ...
      doc.setFontSize(12);
      doc.setTextColor(TEXT_DARK.r, TEXT_DARK.g, TEXT_DARK.b);
      doc.text("Terms and Conditions", pageW / 2, cursorY + 2, {
        align: "center",
      });
      cursorY += 8;
    ...
    if (data.internalNotes) {
      ...
      drawTextBlock(data.internalNotes);
    }
  }
  ...
  // ─── Helper: draw multi-line text block ────────────────
