...
export const deferredExpenseRouter = createTRPCRouter({
  list: p("finance:deferred:read")
    .input(z.object({ state: z.enum(["DRAFT", "RUNNING", "CLOSED"]).optional() }))
    .query(async ({ ctx, input }) => {
      return ctx.db.deferredExpense.findMany({
  ...
  getById: p("finance:deferred:read")
    ...
    .query(async ({ ctx, input }) => {
      const rec = await (ctx.db.deferredExpense as any).findFirst({
  ...
  create: p("finance:deferred:create")
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const rec = await ctx.db.deferredExpense.create({
        data: {
          ...rest,
      ...
      await ctx.db.deferredExpenseSchedule.createMany({
        data: schedule.map((l) => ({ deferredId: rec.id, date: l.date, amount: l.amount })),
      });
      return rec;
    }),

  amortize: p("finance:deferred:update")
    .input(z.object({ scheduleId: z.string() }))
    .mutation(async ({ ctx, input }) => {
      const line = await ctx.db.deferredExpenseSchedule.findFirst({ where: { id: input.scheduleId } });
      if (!line) throw new TRPCError({ code: "NOT_FOUND" });
      await ctx.db.deferredExpenseSchedule.update({ where: { id: input.scheduleId }, data: { isAmortized: true } });
      const deferred = await ctx.db.deferredExpense.findFirst({ where: { id: line.deferredId } });
      if (deferred) {
        await ctx.db.deferredExpense.update({
          where: { id: line.deferredId },
          data: { amortizedAmount: new Decimal(deferred.amortizedAmount.toString()).plus(new Decimal(line.amount.toString())) },
        });
      }
    }),
  ...
  close: p("finance:deferred:confirm")
    .input(z.object({ id: z.string() }))
    .mutation(async ({ ctx, input }) => {
      return ctx.db.deferredExpense.update({
        where: { id: input.id, companyId: ctx.companyId },
        data: { state: "CLOSED" },
      });
    }),
