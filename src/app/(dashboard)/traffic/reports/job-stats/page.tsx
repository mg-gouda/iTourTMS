  ...
  return (
    ...
    <PermissionGuard permission="traffic:report:read">
      ...
      <div className="page-header flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">{t("jobStatsReport")}</h1></div>
      ...
      {isLoading ? <div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-40 w-full" />)}</div> : (
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            ...
            ))}{data?.byStatus.length === 0 && <p className="py-4 text-center text-sm text-muted-foreground">{tc("noData")}</p>}</div></CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle>{t("byServiceType")}</CardTitle></CardHeader>
            ...
            ))}{data?.byServiceType.length === 0 && <p className="py-4 text-center text-sm text-muted-foreground">{tc("noData")}</p>}</div></CardContent>
          </Card>
        </div>
      )}
    </div>
