...
export default function SightseeingEntryDetailPage() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();
  const utils = trpc.useUtils();

  const { data: entry, isLoading } = trpc.tourOps.sightseeing.getById.useQuery({ id });
  const [dialog, setDialog] = useState<SeasonDialog>(emptyDialog());
  const [deleteId, setDeleteId] = useState<string | null>(null);

  const addSeason = trpc.tourOps.sightseeing.addSeason.useMutation({
    onSuccess: () => { utils.tourOps.sightseeing.getById.invalidate({ id }); setDialog(emptyDialog()); toast.success("Season added"); },
    onError: (e) => toast.error(e.message),
  });
  const updateSeason = trpc.tourOps.sightseeing.updateSeason.useMutation({
    onSuccess: () => { utils.tourOps.sightseeing.getById.invalidate({ id }); setDialog(emptyDialog()); toast.success("Season updated"); },
    onError: (e) => toast.error(e.message),
  });
  const deleteSeason = trpc.tourOps.sightseeing.deleteSeason.useMutation({
    onSuccess: () => { utils.tourOps.sightseeing.getById.invalidate({ id }); toast.success("Season deleted"); },
  });
  ...
  function submitSeason() {
    ...
    if (sid) {
      ...
      addSeason.mutate({ entryId: id, ...data });
    }
  }

  if (isLoading) return <div className="p-6 text-muted-foreground">Loading...</div>;
  if (!entry) return <div className="p-6 text-destructive">Entry not found.</div>;
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            <Badge variant="outline">{entry.destinationCode}</Badge>
            <span className="text-xs text-muted-foreground">{dest?.label}</span>
            <h1 className="text-xl font-bold">{entry.nameEn}</h1>
            {entry.nameAr && <span className="text-sm text-muted-foreground" dir="rtl">{entry.nameAr}</span>}
          </div>
      ...
      {entry.seasons.length === 0 && (
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            No seasons yet. Add a season to define the entrance fee.
          </CardContent>
        </Card>
      )}
          ...
          <div key={season.id} className="flex items-center justify-between border rounded px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <Badge variant={season.isActive ? "default" : "secondary"} className="text-sm font-semibold">
                {Number(season.priceEGP).toLocaleString()} EGP
              </Badge>
            </div>
            <div className="flex gap-1">
              <Button size="icon" variant="ghost" onClick={() => setDialog({
                ...
                isActive: season.isActive, priceEGP: Number(season.priceEGP),
              })}>
      ...
      <Dialog open={dialog.open} onOpenChange={(o) => !o && setDialog(emptyDialog())}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label>Entrance Fee (EGP)</Label>
              <Input
                type="number"
                min={0}
                value={dialog.priceEGP}
                onChange={(e) => setDialog((p) => ({ ...p, priceEGP: parseFloat(e.target.value) || 0 }))}
              />
            </div>
            <div className="flex items-center gap-2">
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This season and its price will be permanently removed.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
