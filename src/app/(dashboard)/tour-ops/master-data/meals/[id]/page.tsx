...
import { OPS_MEAL_TYPE_LABELS } from "@/lib/constants/tour-ops";
import type { OpsMealType } from "@prisma/client";

type SeasonDialog = { open: boolean; id?: string; name: string; dateFrom: string; dateTo: string; isActive: boolean; pricePerPax: number };
const emptyDialog = (): SeasonDialog => ({ open: false, name: "", dateFrom: "", dateTo: "", isActive: true, pricePerPax: 0 });

export default function MealRateDetailPage() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();
  const utils = trpc.useUtils();

  const { data: rate, isLoading } = trpc.tourOps.meals.getById.useQuery({ id });
  const [dialog, setDialog] = useState<SeasonDialog>(emptyDialog());
  const [deleteId, setDeleteId] = useState<string | null>(null);

  const addSeason = trpc.tourOps.meals.addSeason.useMutation({
    onSuccess: () => { utils.tourOps.meals.getById.invalidate({ id }); setDialog(emptyDialog()); toast.success("Season added"); },
    onError: (e) => toast.error(e.message),
  });
  const updateSeason = trpc.tourOps.meals.updateSeason.useMutation({
    onSuccess: () => { utils.tourOps.meals.getById.invalidate({ id }); setDialog(emptyDialog()); toast.success("Season updated"); },
    onError: (e) => toast.error(e.message),
  });
  const deleteSeason = trpc.tourOps.meals.deleteSeason.useMutation({
    onSuccess: () => { utils.tourOps.meals.getById.invalidate({ id }); toast.success("Season deleted"); },
  });

  function submitSeason() {
    const { id: sid, open: _o, ...data } = dialog;
    if (sid) updateSeason.mutate({ id: sid, data });
    else addSeason.mutate({ mealRateId: id, ...data });
  }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-bold">{rate.nameEn}</h1>
            <Badge variant="outline">{OPS_MEAL_TYPE_LABELS[rate.mealType as OpsMealType]}</Badge>
            {rate.supplier && <span className="text-sm text-muted-foreground">{rate.supplier.name}</span>}
            <Badge variant="secondary">{rate.currency}/pax</Badge>
          </div>
          ...
          <div key={season.id} className="flex items-center justify-between border rounded px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <Badge variant={season.isActive ? "default" : "secondary"} className="text-sm font-semibold">
                {Number(season.pricePerPax).toLocaleString()} {rate.currency}/pax
              </Badge>
            </div>
            <div className="flex gap-1">
              <Button size="icon" variant="ghost" onClick={() => setDialog({
                ...
                isActive: season.isActive, pricePerPax: Number(season.pricePerPax),
              })}>
      ...
      <Dialog open={dialog.open} onOpenChange={(o) => !o && setDialog(emptyDialog())}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label>Price Per Pax ({rate.currency})</Label>
              <Input type="number" min={0} value={dialog.pricePerPax} onChange={(e) => setDialog((p) => ({ ...p, pricePerPax: parseFloat(e.target.value) || 0 }))} />
            </div>
            <div className="flex items-center gap-2">
